...
// ZJMemBlock
class ZJMemBlock()(implicit p: Parameters) extends LazyModule
  with HasXSParameter with HasWritebackSource {
  override def shouldBeInlined: Boolean = false
  ...
  lazy val module = new ZJMemBlockImp(this)
...
class ZJMemBlockImp(outer: ZJMemBlock) extends LazyModuleImp(outer)
  with HasXSParameter
  with HasFPUParameters
  with HasWritebackSourceImp
  with HasPerfEvents
